...
def create_embeddings(n_nodes, n_features):
    ...
    # return nn.Parameter(torch.zeros(n_nodes, n_features))

def mix_embeddings(h, ndata, emb):
    e = []
...
class PinSage(nn.Module):
    ...
    def __init__(self, num_nodes, feature_sizes, T, restart_prob, max_nodes,
        ...
        if zero_h:
            self.h = nn.Parameter(torch.zeros(num_nodes, self.in_features))
        else:
            self.h = nn.Parameter(torch.randn(num_nodes, self.in_features))
    ...
    def forward(self, G, nodeset):
        ...
        if self.emb is not None:
            h = mix_embeddings(self.h, G.ndata, self.emb)
        else:
            h = self.h
